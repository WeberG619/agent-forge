...
echo ""
echo -e "${BOLD}Agent Forge - Uninstaller${NC}"
echo ""

# Check if installed
if [ ! -d "$INSTALL_DIR" ]; then
    echo -e "${YELLOW}Agent Forge is not installed at $INSTALL_DIR${NC}"
    exit 0
fi
...
if grep -q "Generated by Agent Forge" "$CLAUDE_DIR/CLAUDE.md" 2>/dev/null; then
...
for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc" ]; then
        sed -i '/AGENT_FORGE_DIR/d' "$rc" 2>/dev/null || true
        sed -i '/# Agent Forge/d' "$rc" 2>/dev/null || true
    fi
done
echo -e "${GREEN}[✓]${NC} Cleaned up shell configuration"

echo ""
echo -e "${BOLD}Agent Forge has been uninstalled.${NC}"
