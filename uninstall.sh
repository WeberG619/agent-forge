...
echo ""
echo -e "${BOLD}Claude Power Kit - Uninstaller${NC}"
echo ""

# Check if installed
if [ ! -d "$INSTALL_DIR" ]; then
    echo -e "${YELLOW}Claude Power Kit is not installed at $INSTALL_DIR${NC}"
    exit 0
fi
...
if grep -q "Generated by Claude Power Kit" "$CLAUDE_DIR/CLAUDE.md" 2>/dev/null; then
...
for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc" ]; then
        sed -i '/CLAUDE_POWER_KIT_DIR/d' "$rc" 2>/dev/null || true
        sed -i '/# Claude Power Kit/d' "$rc" 2>/dev/null || true
    fi
done
echo -e "${GREEN}[✓]${NC} Cleaned up shell configuration"

echo ""
echo -e "${BOLD}Claude Power Kit has been uninstalled.${NC}"
